...
namespace windows_client.Mqtt
{
    ...
    public class HikeMqttManager : Listener, HikePubSub.Listener
    {
        ...
        private bool init()
        {
            HikeInstantiation.appSettings.TryGetValue<string>(HikeInstantiation.TOKEN_SETTING, out password);
            HikeInstantiation.appSettings.TryGetValue<string>(HikeInstantiation.UID_SETTING, out topic);
            HikeInstantiation.appSettings.TryGetValue<string>(HikeInstantiation.MSISDN_SETTING, out clientId);
            uid = topic;
