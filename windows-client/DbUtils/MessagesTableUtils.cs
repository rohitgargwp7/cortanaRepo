...
namespace windows_client.DbUtils
{
    public class MessagesTableUtils
    {
        ...
        public static string updateMsgStatus(string fromUser, long msgID, int val)
        {
            try
            {
                using (HikeChatsDb context = new HikeChatsDb(App.MsgsDBConnectionstring + ";Max Buffer Size = 1024"))
                {
                    ...
                    if (message != null)
                    {
                        ...
                        if (message.MessageStatus >= ConvMessage.State.FORCE_SMS_SENT_CONFIRMED && message.MessageStatus < ConvMessage.State.SENT_SOCKET_WRITE)
                        {
            ...
            {
                Debug.WriteLine(string.Format("MessageTableUtils::updateMsgStatus:Exception:{0},StackTrace:{1}", ex.Message, ex.StackTrace));
            }
            return null;
        }
            ...
            using (HikeChatsDb context = new HikeChatsDb(App.MsgsDBConnectionstring))
            {
                ...
                foreach (var message in messageList)
                {
                    ...
                    if (message != null)
                    {
                        ...
                        //hack to update db for sent socket write
                        if ((message.MessageStatus == ConvMessage.State.SENT_SOCKET_WRITE && (int)ConvMessage.State.SENT_CONFIRMED <= val)
                            || (int)message.MessageStatus < val)
                        {
                            if (fromUser == null || fromUser == message.Msisdn)
                            {
                                message.MessageStatus = (ConvMessage.State)val;
