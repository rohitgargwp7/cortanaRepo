...
namespace CommonLibrary.Lib
{
    public class HikePubSub
    {
        ...
        private readonly BlockingQueue _mQueue;
        private Dictionary<string, List<Listener>> _listeners;

        public HikePubSub()
        {
            _listeners = new Dictionary<string, List<Listener>>();
            _mQueue = new BlockingQueue(100);
            try
            {
                _mThread = new Thread(new ThreadStart(startPubSub));
                _mThread.Name = "PUBSUB THREAD";
                _mThread.Start();
            }
            catch (Exception ex)
            {
        ...
        {
            lock (_listeners) // enter synchronization here i.e only one thread can enter this part
            {
                List<Listener> list;
                _listeners.TryGetValue(type, out list);
                if (list == null)
                {
        ...
        public bool publish(string type, object o)
        {
            ...
            _mQueue.Enqueue(new Operation(type, o));
            return true;
        }
        ...
        {
            List<Listener> l;
            _listeners.TryGetValue(type, out l);
            if (l != null)
            {
                l.Remove(listener);
            }
        ...
        private void startPubSub()
        {
            ...
            while (true)
            {
                ...
                lock (_listeners)  // seems not required here
                {
                    _listeners.TryGetValue(type, out list);
                }
                if (list == null)
